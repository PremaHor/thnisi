...
import { getUserRatingSummary, getRatingBreakdown } from "../data/ratingsStore";
import { BARTER_OFFERS } from "../data/barterOffers";
...
export function SellerProfile() {
  ...
  const sellerOffers = BARTER_OFFERS.filter((o) => o.sellerId === sellerId);
  const seller = sellerOffers[0]?.seller;

  const rating = getUserRatingSummary(sellerId ?? "");
  const breakdown = getRatingBreakdown(sellerId ?? "");
  const maxBreakdownCount = Math.max(...(Object.values(breakdown) as number[]), 1);
        ...
        <div className="sticky top-0 z-10 border-b border-border bg-background/95 py-3 pt-safe backdrop-blur">
          <div className="app-container flex items-center gap-2">
            <button
              type="button"
              onClick={() => navigate(-1)}
              className="min-h-[44px] min-w-[44px] -ml-1 flex shrink-0 items-center justify-center rounded-lg hover:bg-secondary transition-colors"
            >
  ...
  const firstOfferId = sellerOffers[0]?.id;
    ...
    <div className="app-screen">
      ...
      <div className="app-container space-y-5 py-6">
        ...
        <div className="flex flex-col items-center text-center gap-3">
          <Avatar size="xl" src={seller.avatar || undefined} />
          <div>
          ...
          {firstOfferId && (
            <Button
              variant="outline"
              size="sm"
              onClick={() => navigate(`/chat/seller-${firstOfferId}`)}
            >
              ...
              {([5, 4, 3, 2, 1] as const).map((score) => {
                ...
                return (
                  <div key={score} className="flex items-center gap-2 text-xs">
                    ...
                    <Star
                      className="h-3 w-3 shrink-0 fill-muted-foreground text-muted-foreground"
                      strokeWidth={0}
                    />
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-3">
          <div className="bg-card border border-border rounded-xl p-3 text-center sm:p-4">
            <div className="text-2xl font-bold mb-1">{seller.completedTrades}</div>
            <p className="text-xs text-muted-foreground">Dokončené směny</p>
          </div>
          <div className="bg-card border border-border rounded-xl p-3 text-center sm:p-4">
            <div className="text-2xl font-bold mb-1">{sellerOffers.length}</div>
                ...
                >
                  ...
                  <div className="min-w-0 flex-1 py-0.5">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      <Badge variant="primary" className="text-[0.65rem] py-0 px-2">
                        {offer.category}
                      </Badge>
